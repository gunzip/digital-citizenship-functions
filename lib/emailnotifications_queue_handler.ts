...
import {
  IEmailNotificationEvent,
  isIEmailNotificationEvent
} from "./models/email_notification_event";
...
export function index(context: IContextWithBindings) {
  ...
  if (
    context.bindings.emailNotification != null &&
    isIEmailNotificationEvent(context.bindings.emailNotification)
  ) {
    // it is an IEmailNotificationEvent
    const emailNotificationEvent = context.bindings.emailNotification;
    context.log(
      `Dequeued email notification|${emailNotificationEvent.message.fiscalCode}`
    );
